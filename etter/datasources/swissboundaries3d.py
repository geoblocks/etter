"""
SwissBoundaries3D data source implementation.

Loads geographic names from swisstopo's swissBOUNDARIES3D dataset and provides
...
# Based on swissBOUNDARIES3D 2026 (https://www.swisstopo.admin.ch/en/landscape-model-swissboundaries3d).
OBJEKTART_TYPE_MAP: TypeMap = {
    "canton": ["Kanton"],
    "municipality": ["Gemeindegebiet"],
    "district": ["Bezirk"],
}
...
def _objektart_to_type(objektart: str) -> str:
    ...
    Args:
        objektart: Raw OBJEKTART value from swissBOUNDARIES3D data

    Returns:
        Normalized type string (e.g., "canton", "municipality", "district")
    """
...
class SwissBoundaries3DSource:
    ...
    If data_path is a directory, automatically loads and concatenates all swissBoundaries3D
    shapefiles (swissBOUNDARIES3D_1_5_TLM_BEZIRKSGEBIET, swissBOUNDARIES3D_1_5_TLM_HOHEITSGEBIET, swissBOUNDARIES3D_1_5_TLM_KANTONSGEBIET) found within.

    IMPORTANT:
        The swissBOUNDARIES3D_1_5_TLM_LANDESGEBIET is NOT read because it contains enclaves of Germany which are not relevant for Swiss geographic names.
        The swissBOUNDARIES3D_1_5_TLM_HOHEITSGRENZE is NOT read because it contains lines already in swissBOUNDARIES3D_1_5_TLM_HOHEITSGEBIET.
    ...
    def _load_from_directory(self) -> None:
        """Load and concatenate all swissBoundaries3D shapefiles from a directory."""
        # Look for the 3 standard swissBoundaries3D shapefiles
        shapefile_names = [
        ...
        if not gdfs:
            raise ValueError(
                f"No swissBoundaries3D shapefiles found in {self._data_path}. Expected: {', '.join(shapefile_names)}"
            )
    ...
    def get_available_types(self) -> list[str]:
        ...
        representing all possible types that swissBoundaries3D data can be classified as.
